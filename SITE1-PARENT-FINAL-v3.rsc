# SITE1-PARENT-FINAL-v3.rsc
# MikroTik RBwAPG-5HacT2HnD - RouterOS 7.12.1
# Full configuration from a clean no-defaults reset.
# Role: transparent Layer-2 NV2 master
# Management: DHCP + 192.168.88.1/24
#
# IMPORTANT:
...
# - NV2 uses nv2-security/nv2-preshared-key; wireless security profiles
#   do not protect an NV2 link.

# ---------- Identity and clock ----------
/system identity set name="SITE1-PARENT"
/system clock set time-zone-autodetect=no time-zone-name=Europe/Gibraltar
/system note set show-at-login=yes note="SITE1 PARENT | NV2 master | 5660 MHz / 20 MHz | management 192.168.88.1"
...
add bridge=bridge-ptp interface=ether1 path-cost=10 comment="Router/modem side"
add bridge=bridge-ptp interface=wlan2 path-cost=20 comment="5 GHz NV2 PTP"

/interface list member
add interface=bridge-ptp list=MGMT

# ---------- Disable unused 2.4 GHz radio on Parent ----------
/interface wireless
set [find default-name=wlan1] disabled=yes comment="Unused on Parent"
...
set [find default-name=wlan2] \
    ...
    comment="SITE1 5 GHz NV2 master"

# ---------- Management addressing ----------
/ip address
add address=192.168.88.1/24 interface=bridge-ptp \
...
# WinBox, SSH, HTTP and HTTPS remain accessible through either the DHCP
# address or the fail-safe 192.168.88.1 address.
/ip service
...
# ---------- Watchdog ----------
# Hardware/software lockup watchdog only. No Internet ping target is used,
# avoiding unwanted reboots during upstream Internet outages.
/system watchdog
...
# /system script run SITE1-Radio-Recovery
/system script
...
# The same two filenames are overwritten nightly to protect the small flash.
/system script
add name=SITE1-Nightly-Backup policy=read,write,policy,test,sensitive source={
    ...
    /system backup save name="SITE1-PARENT-AUTO" dont-encrypt=yes;
    /export show-sensitive file="SITE1-PARENT-AUTO";
    :log info "SITE1: nightly configuration backup completed";
}

/system scheduler
add name=SITE1-Nightly-Backup start-time=03:15:00 interval=1d \
...
# ---------- Final note ----------
:log warning "SITE1-PARENT configuration loaded. Verify wlan2 and DHCP before remote installation."
